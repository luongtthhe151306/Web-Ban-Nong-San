 ...
 * @author Admin
 */
@WebFilter(filterName = "FilterLogin", servletNames = {"LoginServlet"})
public class FilterLogin implements Filter {
    ...
    private void doBeforeProcessing(ServletRequest request, ServletResponse response)
        ...
        }else if(!accname.isEmpty() && pass.isEmpty()){
            error="Nhập mật khẩu!!!";
            request.setAttribute("error", error);
            request.getRequestDispatcher("Login.jsp").forward(request, response);
        }else if(accname.isEmpty() && pass.isEmpty()){
            error="Nhập tên tài khoản và mật khẩu!!!";
            request.setAttribute("error", error);
            request.getRequestDispatcher("Login.jsp").forward(request, response);
        }else{
            boolean f = false;
            for(Account acclist1 : acclist){
                if(acclist1.getAccountName().equals(accname) && acclist1.getPassword().equals(pass)){
                    f = true;
