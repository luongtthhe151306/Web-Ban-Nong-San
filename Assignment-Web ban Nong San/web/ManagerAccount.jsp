                    ...
                    <table style="border: 1px solid; background-color: #fff;margin: 30px 30px 10px 30px;width: 95%;height: 70%;">
                        ...
                        <c:forEach begin="${requestScope.begin}" end="${requestScope.end}" items="${requestScope.allacc}" var="acc">
                            <tr>
                                ...
                                <td style="border: 1px solid;"><a href="AccountPage.jsp?IdA=${acc.getIdA()}&IdC=<%=IdA%>">Update</a></br>
                                    <a href="DeleteAccountServlet?IdC=<%=IdA%>&IdA=${acc.getIdA()}" onclick="if (!confirm('Bạn muốn xóa tài khoản?')) {return false;}">Delete</a></td>
                            </tr>
                        </c:forEach>
                        <tr>
